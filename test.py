...
def main():
    ...
    test_img_ids = sorted(glob(os.path.join(config['data_dir'], 'ISIC2016', 'test', 'images', '*' + img_ext)))
    ...
    with torch.no_grad():
        for input, target, edge, meta in tqdm(test_loader, total=len(test_loader)):
            input = input.cuda()
            target = target.cuda()
            edge = edge.cuda()
            model = model.cuda()
