...
def parse_args():
    ...
    # model
    parser.add_argument('--arch', '-a', metavar='ARCH', default='BGANet')
...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input, target, edge, _ in train_loader:
        input = input.cuda()
        target = target.cuda()
        edge = edge.cuda()
        ...
        if config['deep_supervision']:
            ...
            loss_s = 0
            loss_e = 0
            for output, output_e in zip(outputs, outputs_e):
...
def validate(config, val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for input, target, edge, _ in val_loader:
            input = input.cuda()
            target = target.cuda()
            edge = edge.cuda()
...
def main():
    ...
    # Data loading code
    train_img_ids = sorted(glob(os.path.join(config['data_dir'], config['dataset'], 'train', 'images', '*' + img_ext)))
    ...
    train_dataset = Dataset(
        ...
        img_dir=os.path.join(config['data_dir'], config['dataset'], 'train', 'images'),
        mask_dir=os.path.join(config['data_dir'], config['dataset'], 'train', 'masks'),
        img_ext=img_ext,
        mask_ext=mask_ext,
    ...
    val_dataset = Dataset(
        img_ids=val_img_ids,
        img_dir=os.path.join(config['data_dir'], config['dataset'], 'val', 'images'),
        mask_dir=os.path.join(config['data_dir'], config['dataset'], 'val', 'masks'),
        img_ext=img_ext,
        mask_ext=mask_ext,
        num_classes=config['num_classes'],
    ...
    for epoch in range(config['epochs']):
        ...
        if val_log['iou'] > best_valiou:
            torch.save(model.state_dict(), f'{output_dir}/{exp_name}/val_model.pth')
            best_valiou = val_log['iou']
            best_valdice = val_log['dice']
        ...
        if config['early_stopping'] >= 0 and trigger >= config['early_stopping']:
            torch.save(model.state_dict(), f'{output_dir}/{exp_name}/epoch{epoch}_model.pth')
